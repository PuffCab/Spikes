...
interface AuthContextType {
  ...
  updateUser: (values:{ email: string, username: string | undefined }) => Promise<void>
  loading: boolean
}
...
const defaultValue: AuthContextType = {
  ...
  updateUser: () => {throw new Error("no provider")},
  loading: false
}
...
export const AuthContextProvider = ({ children }: PropsWithChildren) => {
  ...
  const updateUser = async(values: { email: string, username: string | undefined }) => {
    ...
    try {
      const response = await fetch(`${baseUrl}/api/users/update/${user._id}`, requestOptions);
      if (response.ok) {
        const result = await response.json() as User;
        setUser(result);
      }
